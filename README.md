...
[![HPy tests](https://github.com/mburakmmm/hpy-ujson/actions/workflows/hpy.yml/badge.svg)](https://github.com/mburakmmm/hpy-ujson/actions/workflows/hpy.yml)
[![Classic regression tests](https://github.com/mburakmmm/hpy-ujson/actions/workflows/test.yml/badge.svg)](https://github.com/mburakmmm/hpy-ujson/actions/workflows/test.yml)
[![Development status: Alpha](https://img.shields.io/badge/status-alpha-orange.svg)](https://github.com/mburakmmm/hpy-ujson#project-status)
[![License: BSD-3-Clause AND TCL](https://img.shields.io/badge/license-BSD--3--Clause%20AND%20TCL-blue.svg)](./LICENSE.txt)
...
functional `ujson_hpy` module for CPython ABI and Universal ABI hosts.
...
- Supported CPython versions are 3.10 through 3.14. HPy CI currently targets
  Linux and macOS; Windows HPy support is not yet claimed.
- Local validation on July 14, 2026 includes:
...
Install the CPython ABI build of `ujson_hpy` from this checkout:

```sh
python -m pip install .
```

The distribution name is `hpy-ujson`; the import name is `ujson_hpy`.
Universal ABI host artifacts use the explicit build flow below.
...
## Threading and security status

The HPy binding does not intentionally add mutable process-global state.
Threading and free-threading guarantees still depend on the host runtime and
its object model. Mutating an object while another thread serializes it is not
supported.

This fork has not received an independent security audit. The HPy binding is
new native-code surface, and the repository currently provides deterministic
smoke fuzzing rather than continuous coverage-guided fuzzing. Treat untrusted
input and production deployment accordingly; this alpha release is intended
for runtime integration, compatibility testing, and performance evaluation.
See [the security policy](./.github/SECURITY.md) for reporting and current
support status.
...
This repository carries a functional `ujson_hpy` module which ports the UltraJSON
...
The default local package build produces the HPy CPython ABI extension:

```sh
python -m pip install .
```

Use the explicit flow for Universal ABI artifacts:

```sh
UJSON_BUILD_HPY=1 python setup.py --hpy-abi=cpython --hpy-use-static-libs build_ext
```

The build defaults to producing only the `ujson_hpy` extension. Set
`UJSON_BUILD_CPYTHON_EXT=1` if you also want the classic `ujson` extension for
side-by-side comparison. Set `UJSON_BUILD_HPY=0` to request a classic-only
development build.
...
Local run from commit `b960d0a` on July 13, 2026, on
`macOS-26.5.2-arm64-arm-64bit-Mach-O` with
